...
struct spi_fmc_dev {
	...
	struct ad9516_lvds_cmos_channel_spec ad9516_fpga_channels[2];
	struct ad9516_lvds_cmos_channel_spec ad9516_dac_channels[1];

	// tbe fourth line is the device clk that is directly connected to the ad9144 chip in this evaluation board
...
static int spi_fmc_gpio_init(struct spi_fmc_dev *dev)
{
	int status;
...
static int spi_fmc_spi_init(struct spi_fmc_init_param *dev_init)
...
static int spi_fmc_clk_init(struct spi_fmc_dev *dev,
			    struct spi_fmc_init_param *dev_init)
{
	static struct ad9516_platform_data ad9516_pdata;
	int ret;
// the registers of ad9144 and ad9516 are different. 
	...
	dev_init->ad9516_param.ad9516_type = AD9516_1; // the board has AD9516-1 attached to it
...
static int spi_fmc_jesd_init(struct spi_fmc_init_param *dev_init)
{
...
//		.rx_adc = spi_fmc.ad9680_core,
//		.rx_dmac = spi_fmc.ad9680_dmac,
//#ifndef PLATFORM_MB
...
//	};
	...
	iio_axi_dac_init_par = (struct iio_axi_dac_init_param) {
		.tx_dac = spi_fmc.ad9144_core,
		.tx_dmac = spi_fmc.ad9144_dmac,
#ifndef PLATFORM_MB
...
//		return status;
	...
	status = fmcdac_dac_init(&spi_fmc, &fmcdac_init);
	if (status < 0)
		return status;
...
#endif
*/
	status = fmcdac_trasnceiver_setup(&spi_fmc, &fmcdac_init);
	if (status != 0)
		return status;
...
int main(void)
{
	...
	status = fmcdac_setup(&spi_fmc, &fmcdac_init);
	if (status < 0)
		return status;
	...
	/*struct jesd204_topology_dev devs[] = {
		{
			.jdev = spi_fmc.ad9680_jesd->jdev,
			.link_ids = {1},
			.links_number = 1,
		},
		{
			.jdev = spi_fmc.ad9680_device->jdev,
			.link_ids = {1},
			.links_number = 1,
			.is_top_device = true,
		},
...
*/
	struct jesd204_topology_dev devs_tx[] = {
		{
			.jdev = spi_fmc.ad9144_jesd->jdev,
			.link_ids = {1},
			.links_number = 1,
		},
		{
			.jdev = spi_fmc.ad9144_device->jdev,
			.link_ids = {1},
			.links_number = 1,
			.is_top_device = true,
		},
	...
	fmcdac_test(&spi_fmc, &fmcdac_init);

    //status = axi_dmac_init(&spi_fmc.ad9680_dmac, &fmcdac_init.ad9680_dmac_param);
	//if (status)
	//	return status;
	...
	spi_fmc.ad9144_channels[0].sel = AXI_DAC_DATA_SEL_DMA;
	spi_fmc.ad9144_channels[1].sel = AXI_DAC_DATA_SEL_DMA;
	spi_fmc.ad9144_channels[2].sel = AXI_DAC_DATA_SEL_DMA;
	spi_fmc.ad9144_channels[3].sel = AXI_DAC_DATA_SEL_DMA;

#ifdef USE_NCO
	status = ad9144_set_nco(spi_fmc.ad9144_device,62500,1);
	if (status)
		return status;
#endif
	status = axi_dmac_init(&spi_fmc.ad9144_dmac, &fmcdac_init.ad9144_dmac_param);
	if (status)
		return status;
	axi_dac_data_setup(spi_fmc.ad9144_core);
	axi_dac_load_custom_data(spi_fmc.ad9144_core, sine_lut_iq,
	...
	status = axi_dmac_transfer_start(spi_fmc.ad9144_dmac, &transfer_tx);
	if (status)
		return status;
#ifndef ALTERA_PLATFORM
...
#endif
#else
	spi_fmc.ad9144_channels[0].dds_dual_tone = 0;
	spi_fmc.ad9144_channels[0].dds_frequency_0 = 33*1000*1000;
	spi_fmc.ad9144_channels[0].dds_phase_0 = 0;
	spi_fmc.ad9144_channels[0].dds_scale_0 = 500000;
	spi_fmc.ad9144_channels[0].sel = AXI_DAC_DATA_SEL_DDS;
	spi_fmc.ad9144_channels[1].dds_dual_tone = 0;
	spi_fmc.ad9144_channels[1].dds_frequency_0 = 11*1000*1000;
	spi_fmc.ad9144_channels[1].dds_phase_0 = 0;
	spi_fmc.ad9144_channels[1].dds_scale_0 = 500000;
	spi_fmc.ad9144_channels[1].sel = AXI_DAC_DATA_SEL_DDS;
	spi_fmc.ad9144_channels[2].dds_dual_tone = 0;
	spi_fmc.ad9144_channels[2].dds_frequency_0 = 33*1000*1000;
	spi_fmc.ad9144_channels[2].dds_phase_0 = 0;
	spi_fmc.ad9144_channels[2].dds_scale_0 = 500000;
	spi_fmc.ad9144_channels[2].sel = AXI_DAC_DATA_SEL_DDS;
	spi_fmc.ad9144_channels[3].dds_dual_tone = 0;
	spi_fmc.ad9144_channels[3].dds_frequency_0 = 11*1000*1000;
	spi_fmc.ad9144_channels[3].dds_phase_0 = 0;
	spi_fmc.ad9144_channels[3].dds_scale_0 = 500000;
	spi_fmc.ad9144_channels[3].sel = AXI_DAC_DATA_SEL_DDS;
	axi_dac_data_setup(spi_fmc.ad9144_core);
#endif
/*
	...
	//status = axi_dmac_transfer_start(spi_fmc.ad9680_dmac, &transfer_rx);
	//if(status)
	//	return status;
	//status = axi_dmac_transfer_wait_completion(spi_fmc.ad9680_dmac, 1000);
	//if(status)
	//	return status;
//#ifndef ALTERA_PLATFORM
...
//		       i, (*(data + i) & 0xFFFF), (*(data + i) >> 16));
	...
	status = spi_fmc_iio_init(&spi_fmc, &spi_fmc_init);
	if (status != 0)
		return status;

	spi_fmc_remove(&spi_fmc);
