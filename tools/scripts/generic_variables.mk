...
HARDWARE ?= $(wildcard *.xsa) $(wildcard *.hdf) $(wildcard *.sopcinfo) \
            $(wildcard *.ioc) $(wildcard pinmux_config.c)

# If platform not set, get it from HARDWARE file
ifeq ($(PLATFORM),)
  ifneq ($(findstring .xsa,$(HARDWARE)),)
    PLATFORM = xilinx
  else ifneq ($(findstring .hdf,$(HARDWARE)),)
    PLATFORM = xilinx
  else ifneq ($(findstring .sopcinfo,$(HARDWARE)),)
    PLATFORM = altera
  else ifneq ($(findstring .ioc,$(HARDWARE)),)
    PLATFORM = stm32
  else ifneq ($(findstring pinmux_config.c,$(HARDWARE)),)
    PLATFORM = aducm3029
  else
